        ...
        if self.alive == 1:
            if not(abs(self.enemy_pos_x - player_x) <= 25 and abs(self.enemy_pos_y - player_y) <= 50):
                ...
                if self.repeat_enemy_anim == 4:
                    self.enemy_anim_count+=(1*(self.enemy_anim_count != 7)-self.enemy_anim_count*(self.enemy_anim_count == 7))
                    self.repeat_enemy_anim = 0
                ...
                if self.repeat_anim_fight == 4:
                    self.enemy_fight_anim_count+=(1*(self.enemy_fight_anim_count != 5)-self.enemy_fight_anim_count*(self.enemy_fight_anim_count == 5))
                    self.repeat_anim_fight = 0
            ...
            if self.anim_die_count != 4:
                ...
                if self.repeat_anim_die == 4:
                    self.anim_die_count+=(1*(self.anim_die_count != 4)-self.anim_die_count*(self.anim_die_count == 4))
                    self.repeat_anim_die = 0
...
while running:
    ...
    if wave_enemy_alive(enemys3) == 0:
        wave = 4
    if wave_enemy_alive(enemys4) == 0 and ((enemys4[7].repeat_anim_die == 4 and enemys4[7].anim_die_count==3) or (enemys4[6].repeat_anim_die == 4 and enemys4[6].anim_die_count==3) or
         (enemys4[5].repeat_anim_die == 4 and enemys4[5].anim_die_count==3) or (enemys4[4].repeat_anim_die == 4 and enemys4[4].anim_die_count==3)):
        wave = 6
    ...
    if not(keys[pygame.K_e]):                                                                       # Передвижение
        fight_anime_count = 0
        fight_anime_1 = 0
        if keys[pygame.K_a] and keys[pygame.K_w] and player_x > 40 and player_y > 40 and allow_move(player_x, player_y) != 1 and allow_move(player_x, player_y) != 2:
            player_x -= player_speed/(2**0.5)
            player_y -= player_speed/(2**0.5)
        elif keys[pygame.K_d] and keys[pygame.K_w] and player_x < 710 and player_y > 40 and allow_move(player_x, player_y) != 1 and allow_move(player_x, player_y) != 4:
            player_x += player_speed/(2**0.5)
            player_y -= player_speed/(2**0.5)
        elif keys[pygame.K_a] and keys[pygame.K_s] and player_y < 510 and player_x > 40 and allow_move(player_x, player_y) != 2 and allow_move(player_x, player_y) != 3:
            player_x -= player_speed/(2**0.5)
            player_y += player_speed/(2**0.5)
        elif keys[pygame.K_d] and keys[pygame.K_s] and player_y < 510 and player_x < 710 and allow_move(player_x, player_y) != 4 and allow_move(player_x, player_y) != 3:
            player_x += player_speed/(2**0.5)
            player_y += player_speed/(2**0.5)
    ...
    if repeat_walk_anim == 4:                                                                       # Измения счётчиков задержки кадров анимации
        player_anime_count += (1*(player_anime_count != 7)-player_anime_count*(player_anime_count == 7))
        repeat_walk_anim = 0
    if repeat_fight_anim == 6:
        fight_anime_count += (1*(fight_anime_count != 7)-fight_anime_count*(fight_anime_count == 7))
        repeat_fight_anim = 0
